...
rmt_item32_t items[LED_CNT * 24 + 1];
pixel_t pixels[LED_CNT];

static char *colorOrder;
 ...
 * a logic 0 for 0.6us
 */
static void set_item_1(rmt_item32_t *pItem) {
    assert(pItem != 0);
    pItem->level0    = 1;
    pItem->duration0 = 10;
    pItem->level1    = 0;
 ...
 * a logic 0 for 0.8us
 */
static void set_item_0(rmt_item32_t *pItem) {
    assert(pItem != 0);
    pItem->level0    = 1;
    pItem->duration0 = 4;
    pItem->level1    = 0;
 ...
 */
static void set_terminator(rmt_item32_t *pItem) {
    assert(pItem != 0);
    pItem->level0    = 0;
    pItem->duration0 = 0;
    pItem->level1    = 0;
 ...
 */
static uint8_t get_channel_value_by_type(char type, pixel_t pixel) {
    switch(type) {
        case 'r':
        case 'R':
            return pixel.red;
 ...
 */
void ws2812_init(gpio_num_t dinPin, uint16_t ipixelCount, int ichannel) {

    pixelCount = ipixelCount;
    channel    = (rmt_channel_t)ichannel;
...
void ws2812_show() {
    ...
    for (int i=0; i<pixelCount; i++) {
        uint32_t currentPixel =
                (get_channel_value_by_type(colorOrder[0], pixels[i]) << 16) |
                (get_channel_value_by_type(colorOrder[1], pixels[i]) << 8)  |
                (get_channel_value_by_type(colorOrder[2], pixels[i]));
     ...
     //   ESP_LOGD(LOG_TAG, "Pixel value: %x", currentPixel);
        for (int j=23; j>=0; j--) {
            ...
            if (currentPixel & (1<<j)) {
                set_item_1(pCurrentItem);
            } else {
                set_item_0(pCurrentItem);
            }
            pCurrentItem++;
        }
    }
    set_terminator(pCurrentItem); // Write the RMT terminator.

    // Show the pixels.
    ESP_ERROR_CHECK(rmt_write_items(channel, items, pixelCount*24, 1 /* wait till done */));
} // show
 ...
 * for example "RGB".
 */
void ws2812_set_color_order(char *icolorOrder) {
    if (colorOrder != 0 && strlen(colorOrder) == 3) {
...
void ws2812_set_led_u32(uint16_t index, uint32_t pixel) {
    ...
    pixels[index].red   = pixel & 0xff;
    pixels[index].green = (pixel & 0xff00) >> 8;
    pixels[index].blue  = (pixel & 0xff0000) >> 16;
} // setPixel
...
void ws2812_set_led_hsb(uint16_t index, uint16_t hue, uint8_t saturation,  uint8_t brightness) {
    ...
    pixels[index].red   = (uint8_t)(new_red*255);
    pixels[index].green = (uint8_t)(new_green*255);
    pixels[index].blue  = (uint8_t)(new_blue*255);
} // setHSBPixel
