 ...
 */
void WS2812(gpio_num_t dinPin, uint16_t ipixelCount, int channel) {
    /*
    if (pixelCount == 0) {
...
void show() {
    ...
    for (int i=0; i<pixelCount; i++) {
        uint32_t currentPixel =
                (getChannelValueByType(colorOrder[0], pixels[i]) << 16) |
                (getChannelValueByType(colorOrder[1], pixels[i]) << 8)  |
                (getChannelValueByType(colorOrder[2], pixels[i]));
 ...
 * for example "RGB".
 */
void setColorOrder(char *icolorOrder) {
